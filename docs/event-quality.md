...
本页说明活动如何归并、来源如何对账。先看 [Nova 实例](event-layers-example.md)理解阅读结果，查询接口见[分层事件查询](event-views.md)。当前规则为 `experience_view_v1_6` / `experience_recap_v1_3`，精确代码和资源哈希随快照保存；适用版本见[文档入口](index.md)。
...
资源依据保存在 `experience_resources.json` 的 `reviewed_activity_rules`，包含官方 tuning 的包位置、TGI、源／展开 XML 的 SHA-256 和类结构。覆盖范围包括生活动作、电脑／电影变体、烹饪／烧烤、游泳／日光浴、清洁和社交等精确身份；不能证明第三方 MOD 没有覆盖资源。逐版核验数量见[验证摘要](validation.md)，当前使用以下边界：

- 烹饪／烧烤通过实际父实例链归入制作活动，并保留成品关联；拿餐、进食和洗碗分别保留。
- 配方 router、自主烧烤／游泳／午睡入口不直接写成已执行活动；自主选择仍保留，配方选择用 `decision_role=action` 表达活动方向。
- 游泳与躺椅姿态作为支持证据；起止不一致或可见时不套用内部洗浴姿态的严格规则。
- `[Join]GroupCooking_Mentor` 按精确代理身份作为独立人物的指导动作保留，不视为食物制作者，也不普遍剥离 `[Join]` 前缀。
- 名称回退释义保留来源、原名和缺失参数；电影反应的资源身份不能证明具体观影结果。
