...
从本仓库根目录运行，Python 3.10+，仅标准库：

```powershell
python tools/audit_reference_data.py --sources-root C:\sources --output docs/research/2026-09-11-reference-audit.json
```

脚本仅读取 Atlas 数据和 Experience 的三个既有样本，不导入或执行参考项目代码。若源文件不是合法 JSON/事件结构，检查会报错，不静默跳过坏行。
