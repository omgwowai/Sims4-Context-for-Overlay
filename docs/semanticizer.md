...
2026-09-16 更新：0.7.0 增加 [Autonomy 决策表达](autonomy-capture.md)。概览显示赢家、提交门槛与层数，`rendered.history[].decision_details` 和游戏详情按层显示候选、原概率、评分组成、筛除计数和缺失字段。按权重／均匀／确定性选择分别表达，入队不等于完成，倍率与贡献不混成百分比。
...
2026-09-16 补充 `rendered.resource_details` 与游戏详情正文、整数金额／短时间／游戏星期格式，并细分未解析原因。旧资源缺少类型时仅做唯一目录匹配的静态参考，不回填旧名称或参数；情绪基础说明不冒充已核实的客户端覆盖。范围及证据见[补充验证](validation/2026-09-16-semantic-supplements.md)。
...
客户端拥有完整的本地化语法。本版支持整数金额、短时间和 EA 游戏星期的确定性中文格式；SIM_LIST、小数金额、其余日期语法、年龄条件、自定义代词和姓名前缀等仍未完整支持。无法确认的部分显示 `〈未解析：表达式〉`，同时保留模板、参数和分类原因，不标记为完整成功。原始用户文本不作为模板执行。protobuf token 类型优先读取运行环境的枚举描述，避免把新版编号按旧参考源码误读。
...
具体执行命令见[离线翻译操作说明](offline-translation.md)：包括整轮 JSONL 的事件修订整理、普通 JSON 数据包转换，以及使用本地资源目录重新解析。
