...
适用 ContextOverlay **0.10.9**；API / SDK **2.2.0**。四层查询入口、分页和原始记录格式不变。规则版本为 `experience_view_v1_6` / `experience_recap_v1_3`，具体规则文件和代码哈希随快照保存。0.10.7 调整内存预算，0.10.8 按预算复用解码结果，0.10.9 移除来源日志的独立字节上限；这些调整均未修改分类或总结规则。
