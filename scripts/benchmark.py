"""Measure current retention, FIFO churn and query latency without EA.
...
from tool_support import process_memory, report as emit_report
...
def main():
    ...
    recovered = read_journal(args.journal, event_scope="retained", include_observations=False)
    ...
    for i in range(total):
        ...
        if (i + 1) % 25000 == 0 or i + 1 in (args.capacity, total):
            row = {"accepted_events": i + 1, "retained": len(recorder.events), "evicted": recorder.index.evicted,
    ...
    if not recorder.paused:
        ...
        assertions["expected_fifo_evictions"] = recorder.index.evicted == args.extra
    ...
    if not recorder.paused:
        ...
        for _ in range(7):
            ...
            recorder.history(actor, limit=50)
            samples.append((time.perf_counter() - started) * 1000)
        queries["recent_median_ms"] = statistics.median(samples)
        started = time.perf_counter()
        page = recorder.query_history(actor, page_size=50, from_ticks=total - min(args.capacity, 10000))
        queries["first_page_ms"] = (time.perf_counter() - started) * 1000
        queries["total_matches"] = page["total_matches"]
        queries["candidates_examined"] = page["candidates_examined"]
        count, samples = len(page["events"]), []
        cursor = page["cursor"]
        while page["has_more"]:
            started = time.perf_counter()
            page = recorder.history_page(page["next_cursor"])
            samples.append((time.perf_counter() - started) * 1000)
            count += len(page["events"])
        queries["next_page_median_ms"] = statistics.median(samples) if samples else None
        assertions["pagination_complete"] = count == page["total_matches"]
        recorder.close_query(cursor)
    final_memory = process_memory()
    report = {"module_version": VERSION, "python": sys.version, "kind": "offline_history_benchmark", "event_scope": "retained", "shape": args.shape,
              "source_journal_sha256": recovered["sha256"],
              ...
              "assertions": assertions, "queries": queries, "recorder": state,
    ...
    emit_report(report, {"state": state["state"], "error": state["error"], "event_scope": "retained",
                         "assertions": assertions, "queries": queries}, args.output, [args.journal])
