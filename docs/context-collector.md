...
版本：v0.1。日期：2026-09-14。状态：当前设计草案，尚未实现或游戏实测。
...
第一轮建议读取指定 Sim 的身份、名称、地块、游戏时间、当前运行交互及其关联物件身份。需求、Buff、关系和物件状态按首批场景逐项增加，不把整个分类目录一次性变成必需采集范围。

目标绑定在请求开始时解析并固定；返回包记录实际实体，不能用后来改变的“当前选中 Sim”解释旧结果。SimInfo、已实例化 Sim、地块物件和库存物件的查找范围分别说明。

## 3. Profile 与请求

Profile 表达目标绑定、必需/可选字段、是否包含历史及历史范围、表示方式与读取限制。允许仅原始数据、仅可读表示或二者组合；具体 JSON 字段在真实输入验证后确定。
...
可用能力包括字段浏览、Profile 校验、当前状态查询、按需组合历史及导出。示意能力不是已经注册的游戏命令或 HTTP 端点。
...
待核定：首批具体字段、目标选择规则、请求与数据包 schema、预算、游戏触发命令和导出目录。旧原型字段和输出样例保存在[归档](archive/README.md)，不直接冻结为新接口。
