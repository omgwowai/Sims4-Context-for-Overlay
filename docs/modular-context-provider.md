...
当前源码为 0.6.0 试用版，删除连续数值定时历史采样，沿用[公共 API v1 与 SDK](public-api-v1.md)作为三个模块的外部消费入口，仍以同一个脚本 MOD 提供数据；完成离线契约验证，尚未安装或实机验收。已安装的 0.3.2 具备状态／历史查看窗口，用户已确认布局，见[窗口使用说明](inspector-manual-test.md)。
...
- **接口状态**：跨 MOD 规范以[公共 API v1](public-api-v1.md)为准；其他设计文档中的示意字段不是已实现的 EA API 或公共承诺。新版本仍需真实消费 MOD 的游戏验收。

历史事件本身属于首轮核心能力。长期记忆、印象卡、遗忘与角色认知是后续消费者；记录参与关系不意味着已实现完整的角色知识模型。

当前 Context 限定当前地块实例；历史按本地发生证据保留参与者身份，不持续追踪场外角色。0.6.0 删除全部连续定时采样，只采明确 Loot 操作内真实数值效果，当前值按请求读取。具体社交动作默认主要层，技术步骤内部层，明确关联的效果进入动作详情。数量达到 200,000 后 FIFO 淘汰并明示范围；内存、写入或采集错误仍显式暂停。当前完整决定见[实施约定](event-expansion-plan.md)，能力限制见[覆盖说明](event-coverage-0.6.0.md)。
