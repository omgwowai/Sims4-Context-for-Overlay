...
0.8.0 起提供 [API／SDK v2](public-api-v2.md)：下游可追加自由 JSON 外部事件，与游戏采集记录共用历史、FIFO 和持久化。上游只管理来源、生产者、实体关联及接收时点，不约定叙事内容协议。外部关联不等于原生游戏事实或当前观测。默认混合历史，可按来源／生产者筛选，也可查询无实体记录。
...
`get_context` 的当前字段只查询活动地块内已实例化的 Sim／物件，默认字段为身份、位置、时间、交互、需求、Buff、关系和常用物件状态；具体字段选择见 API。查询不创建关系、不产生生活历史，也不持续扫描场外实体。

`get_camera_view` 按请求枚举当前区域已实例化实体，返回近似视锥命中的摘要，包含地块外和不同楼层；它不扩大 Context 或历史采集范围。`camera_view.py` 负责无 EA 依赖的几何和覆盖计数，EAAdapter 读取原始管理器条目、相机与尺寸代理；游戏线程内最多处理 10,000 个条目，隐藏对象也占预算。`camera_state.py` 的生命周期 Hook 只记录最近同步的常数大小元数据，不扫描对象、不保存查询历史。相机、几何和名称各自的准确性边界见[接口约定](camera-view.md)。
...
游戏采集事件类型为 `interaction`、`state_change`、`game_event`，`origin=game`；生活事件的 `category` 与 `field` 同值，可用于历史筛选。外部记录为 `external_event`、`origin=external`，payload 原样保存，不参与游戏语义解释。没有记录不证明没有发生。
...
0.9.0 起正常旅行延续同一 session、Recorder 和 Journal，保留 FIFO、序号、查询快照、增量 checkpoint 和外部去重表。识别依据是 EA `game_services.service_manager.is_traveling`，下一次加载须仍为同一个 GameServiceManager 实例；不按相同存档名、地块 ID 或游戏时间猜测连续性。读档、回主菜单、游戏重启和 `co.restart` 开启新 session；磁盘旧运行不自动重载或合并。

旅行卸载时关闭旧 Hook、alarm 和窗口，标记实体退出观测，刷新日志；加载期间 API 暂不可用。新地块重新建立采集对象，每次加载增加 `zone_visit`，交互事件 ID 使用独立的 visit 命名空间，避免 EA ID 复用覆盖旧事件。新事件附带 `observation_scope`（记录时 zone/lot）与 `zone_visit`；已有交互在边界的最后观测结果保持原样，不推测其完成、取消或场外后续。日志用 `zone_exit/zone_entry` 描述边界，保留历史可按已知实体 ID 查询，即使实体在先前地块。当前 Context 的读取范围仍限定当前地块。

查询的 120 秒现实时间 TTL 在加载时继续计时；所有容量／去重／输出预算贯穿整个会话，不因旅行重置。旅行途中退出、服务实例更换或恢复失败时关闭旧日志，不能将旧会话错误续接到新存档。旅行不重读配置，配置变更通过 `co.restart` 生效。

增量读取以已接收序号为界，返回一轮内每个事件的最新修订，页面冻结。checkpoint 在最后一页提供，由下游完成处理后提交；页游标与持续进度不同。被淘汰事件的最后接收序号用于保守检查缺口，防止旧事件最近更新后淘汰却被漏报。索引按实体／生产者／全局维护，增量从最近更新的末端扫描到已处理位置；外部写入限流与去重的边界见 API。
