# ContextOverlay 0.5.0：数据与下游 MOD 接入

ContextOverlay 在同一个脚本 MOD 中提供当前状态采集、事件记录和中文解释。0.5.0 新增 **公共 API 1.0.0 + Python SDK 1.0.0**，下游可以直接取得 Context，无须使用控制台或先导出文件。API 与 SDK 已完成离线契约验证；真实跨 MOD 接入和 0.4.0 引入的名称解析仍待实机测试。

玩家目前安装及飞书附件仍为 0.3.2，使用公共 API 必须先升级提供方到 0.5.0 或兼容的后续版本。源码和本地候选包的更新不代表已安装或发布。
...
当前只观察地块内已实例化的实体；默认关闭连续需求变化历史，当前需求可随时请求。200,000 为事件数量上限，同时有内存、查询快照、写入队列和磁盘预算。过载明确报告，不能理解为无限历史或完整世界记忆。
