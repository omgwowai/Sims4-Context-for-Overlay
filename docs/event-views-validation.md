...
适用 **MOD 0.10.0 / API、SDK 2.2.0 / schema 2 / event_views_v1**。本轮已做自动化与离线样本检查，新的游戏内调用、旅行和负载仍待手动验收；当前结果见[验证记录](validation.md)。本页可以由下游 MOD 开发者执行，也可以由测试者手动加载游戏后从终端发送请求，不需要 Computer Use。
...
1. 从当前源码构建并安装 0.10.0，步骤见[开发说明](development.md#检查与构建)。已发布的 0.9.0 ZIP 没有新接口。安装前退出游戏。
