...
def digest(organized, game_version=None):
    ...
    for unit in organized["activities"]:
        row = {"id": unit["id"], "action": name(unit["action"]), "roles": people(unit["roles"]),
               ...
               "time": [at(unit["started"]), at(unit["ended"])], "execution": unit["execution"],
               "first_observed": at(unit.get("first_observed")), "queued": at(unit.get("queued")),
               "root_exit": unit["exit"], "root_result_branch": unit["result_branch"],
               "evidence": unit["evidence"], "results": [], "decisions": []}
        resolution = resolve_label(unit["action"], "interaction", unit.get("action_tuning"), game_version)
        if resolution["basis"] != "observed_name":
            row.update(action=resolution["text"], observed_action_name=row["action"],
                       action_label_basis=resolution["basis"], action_label_resolution=resolution)
        for topic in unit["topics"]:
            row.setdefault("topics", []).append({"action": resolve_label(topic["action"], "interaction", topic.get("action_tuning"), game_version)["text"], "roles": people(topic["roles"]),
