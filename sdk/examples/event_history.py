"""0.6.0 additive event API; invoke from a simulation-thread callback.
...
def read_event_pages(sim_id):
    client = Client()
    if "events.gameplay" not in client.get_api_info().get("capabilities", []):
        return {"available": False, "reason": "provider_needs_event_expansion"}
