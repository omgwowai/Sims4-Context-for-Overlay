...
中文输出仍是确定性转换，不调用大语言模型。游戏名称优先来自 **EA 运行时名称接口 → LocalizedString 的 hash 与 tokens → 中文 STBL**；事件的排队、运行、结束、触发来源和采样差异则由项目规则组织成逐条说明。
...
事件解释仍只表达已观测事实：自然结束不能扩写成“做饭成功”“吃饱”或“关系改善”；采样区间内的差异不能仅凭时间接近归因于某次行为。名称解析不会改变事件身份、阶段、参与者、数值、时间或结果。
